import pygame
import time
import random
from pygame import mixer


pygame.init()
 

score_color = (255, 255, 255)
snake_color = (255, 102, 178)
message_color = (213, 50, 80)
food_color = (0, 255, 0)
#background_color = (0, 128, 255)

# View dimensions
dis_width = 740
dis_height = 740

dis = pygame.display.set_mode((dis_width, dis_height))
pygame.display.set_caption('Snake Game by G0nghiG0n')

# Background Img
background = pygame.image.load('./background_1.png')

clock = pygame.time.Clock()
 
snake_block = 10

# Score / Quit
font_style = pygame.font.SysFont("bahnschrift", 35)
score_font = pygame.font.SysFont("menlo", 35)

# background music 
mixer.music.load('./retrogame_1.ogg')
mixer.music.play(-1)


def Your_score(score):
    value = score_font.render("Score: " + str(score), True, score_color)
    dis.blit(value, [20, 15])
 
 
 
def snake(snake_block, snake_list):
    for x in snake_list:
        pygame.draw.rect(dis, snake_color, [x[0], x[1], snake_block, snake_block])
 
 
def message(msg, color):
    mesg = font_style.render(msg, True, color)
    dis.blit(mesg, [dis_width / 6, dis_height / 3])
 
 
def gameLoop():
    game_over = False
    game_close = False
 
    x1 = dis_width / 2
    y1 = dis_height / 2
 
    x1_change = 0
    y1_change = 0

    snake_speed = 15
    snake_List = []
    Length_of_snake = 1

    
 
    # add food randomly 
    foodx = round(random.randrange(0, dis_width - snake_block) / 10.0) * 10.0  
    foody = round(random.randrange(0, dis_height - snake_block) / 10.0) * 10.0
 
    while not game_over:
        
        while game_close == True:
            dis.blit(background, (0,0))
            #dis.fill(background_color)
            message("Game Over! Press C-Play Again or Q-Quit", message_color)
            Your_score(Length_of_snake - 1)
            pygame.display.update()
 
            for event in pygame.event.get():                    # play again or quit
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_q:
                        game_over = True
                        game_close = False
                    if event.key == pygame.K_c:
                        gameLoop()
 
        for event in pygame.event.get():                        # game movements
            if event.type == pygame.QUIT:
                game_over = True
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    x1_change = -snake_block
                    y1_change = 0
                elif event.key == pygame.K_RIGHT:
                    x1_change = snake_block
                    y1_change = 0
                elif event.key == pygame.K_UP:
                    y1_change = -snake_block
                    x1_change = 0
                elif event.key == pygame.K_DOWN:
                    y1_change = snake_block
                    x1_change = 0
 
        if x1 >= dis_width or x1 < 0 or y1 >= dis_height or y1 < 0:     # game over if you hit borders
            game_close = True
        
        x1 += x1_change
        y1 += y1_change
        dis.blit(background, (0,0))
        #dis.fill(background_color)
        pygame.draw.rect(dis, food_color, [foodx, foody, snake_block, snake_block])
        snake_Head = []                                                 
        snake_Head.append(x1)
        snake_Head.append(y1)
        snake_List.append(snake_Head)
        
        if len(snake_List) > Length_of_snake:
            del snake_List[0]
 
        for x in snake_List[:-1]:                                       # game over if you hit yourself
            if x == snake_Head:
                game_close = True
 
        snake(snake_block, snake_List)
        Your_score(Length_of_snake - 1)
 
        pygame.display.update()
 
        if x1 == foodx and y1 == foody:                                 
            foodx = round(random.randrange(0, dis_width - snake_block) / 10.0) * 10.0
            foody = round(random.randrange(0, dis_height - snake_block) / 10.0) * 10.0
            Length_of_snake += 1
            if snake_speed < 35:                                        # increase the size of the snake
                snake_speed += 1
            #print(snake_speed)
 
        clock.tick(snake_speed)

    pygame.quit()
    quit()
 
 
gameLoop()
# comment X